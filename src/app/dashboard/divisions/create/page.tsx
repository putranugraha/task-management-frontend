...
function CreateDivisionPageContent() {
  ...
  useEffect(() => {
    // Code is generated by backend. Keep field read-only and informational only.
    // Leave it empty so users don't need to think about it.
    setForm((s) => ({ ...s, code: "" }));
  }, []);
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setForm((s) => {
      ...
      return { ...s, [name]: value } as FormState;
    });
  };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: Record<string, any> = {
        code: null, // auto-generated server-side
        name: form.name,
        description: form.description || null,
      };
      const created = await apiRequest<any>("POST", "/api/divisions", payload);
      const createdUnwrapped =
        created && typeof created === "object" && "data" in created
          ? (created as any).data
          : created;
      const createdCode = createdUnwrapped?.code ? String(createdUnwrapped.code) : null;
      const okMsg = createdCode
        ? `Division berhasil ditambahkan dengan code ${createdCode}.`
  ...
  const checklistItems = useMemo(() => {
    return [
      ...
      { key: "code", label: "Code dibuat otomatis untuk identifikasi", completed: Boolean(form.name) },
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                value={form.code}
                readOnly
                className="h-11 w-full cursor-not-allowed rounded-xl border border-slate-200 bg-neutral-50 px-4 text-sm font-medium text-neutral-600 shadow-inner"
                placeholder="Auto"
                autoComplete="off"
              />
            </div>
